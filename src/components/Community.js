...
const Community = () => {
  ...
  useEffect(() => {
    ...
    const fetchUser = async () => {
      try {
        const response = await fetch('https://smartclassroom.onrender.com/api/auth/getuser', {
          method: 'POST',
          headers: {
    ...
    const fetchMessages = async () => {
      try {
        const response = await fetch('https://smartclassroom.onrender.com/api/auth/getcommunity', {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            'auth-token': localStorage.getItem('token'),
  ...
  const handleSendMessage = async () => {
    if (user && message.trim() !== '') {
      try {
        const response = await fetch('https://smartclassroom.onrender.com/api/auth/postcommunity', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'auth-token': localStorage.getItem('token'),
