...
const TAssignment = () => {
    ...
    const fetchAssignments = async () => {
        try {
            const res = await fetch('https://smartclassroom.onrender.com/api/auth/getassignment', {
                method: 'GET',
                headers: {
    ...
    const handlePostAssignment = async () => {
        ...
        try {
            const res = await fetch('https://smartclassroom.onrender.com/api/auth/postassignment', {
                method: 'POST',
                headers: {
                    'auth-token': localStorage.getItem('token'),
                    'auth': localStorage.getItem('classToken')
                    ...
                    {submittedAssignments.map((assignment, index) => (
                        <ListItem key={index}>
                            <ListItemText
                                ...
                                secondary={
                                    assignment.file && (
                                        <Link href={`https://smartclassroom.onrender.com/files/${assignment.file}`} target="_blank" rel="noopener noreferrer">
                                            {assignment.file}
                                        </Link>
                                    )
                                }
                                ...
                                {assignment.students.map((student, i) => (
                                    <ListItem key={i}>
                                        <ListItemText
                                            ...
                                            secondary={
                                                student.file && (
                                                    <Link href={`https://smartclassroom.onrender.com/files/${student.file}`} target="_blank" rel="noopener noreferrer">
                                                        {student.file}
                                                    </Link>
                                                )
                                            }
