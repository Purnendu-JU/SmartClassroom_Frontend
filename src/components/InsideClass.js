  ...
  useEffect(() => {
    const fetchRoleAndClass = async () => {
      try {
        ...
        if (token && classToken) {
          const resRole = await fetch('https://smart-classroom-backend.vercel.app/api/auth/getrole', {
            headers: {
              'auth-token': token,
              'auth': classToken
            },
          ...
          const resClass = await fetch('https://smart-classroom-backend.vercel.app/api/auth/getclass', {
            headers: {
              'auth': classToken
            },
          });
